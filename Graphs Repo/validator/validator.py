class Validator(object):
    ...
    def valid_edge(self, x, y, edges):
        ...
        if int(x) not in edges or int(y) not in edges:
            er += "Please insert some valid vertices, these do not exist in the graph! "
            raise ValueError(er)
    ...
    def vertex_valid(self, v, out):
        ...
        try:
            ...
            raise ValueError(err)
        if int(v) not in out:
            err += "Please input a valid vertex! The vertex must be in the graph! "
            raise ValueError(err)
    ...
    def add_edge(self, x, y, edges):
        ...
        if (x, y) in edges:
            errors += "This edge already exists! Please input an other one!  "
    ...
    def remove_edge(self, x, y, edges):
        ...
        errors = "An error occured! "
        if (x, y) not in edges:
            errors += "This edge do not exist! Please input an existing one! "
            raise ValueError(errors)
